...
## INS-015: OAuth Testing mode は週次失効する構造、Production 公開が唯一の根治策
**発生**: 2026-06-06 | **Arousal**: 0.7 | **ドメイン**: oauth / auth-architecture
Google OAuth の "Testing" モードは refresh トークンが **7日で自動失効**する仕様。`~/.gmail-mcp/credentials.json` で 2026-04-20 / 2026-05-27 / 2026-06-04 と週次タイミングで `invalid_grant` 発生し、毎週手動 reauth が必要だった。
**根本対策**: Google Cloud Console で OAuth consent screen を Testing → 本番環境に切替えると refresh トークンが **6ヶ月未使用または revoke まで永続化**される。自己利用のみなら検証審査は不要で「未確認のアプリ」警告のまま運用可（Advanced → 続行で通せる）。
**実例**: gmail-mcp app を 2026-06-06 に本番環境化、検証審査はスキップ。次の検証ポイントは ~6/13-15 頃（前回 7 日サイクルなら失効していたタイミング）。
**応用**: 自宅環境/個人プロジェクトで Google OAuth を使うすべての MCP/スクリプトに同じ判断が適用可能。「テスト用だから Testing でいい」は週次失効を意味するので、運用に乗せる時点で Production 化を即決すべき。Drive/Gmail/Calendar 系の自動化全般で必要な前提知識。
