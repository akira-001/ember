...
```
┌──────────────────────┐   ┌──────────────────────┐
│    cogmem.toml       │   │    store.py 定数      │
│  [behavior]          │   │  _DEFAULT_SKILL_      │
...
組み込みデフォルト（`store.py` 定数）と ユーザー定義（`cogmem.toml`）をマージした skill_triggers を、リアルタイム層と事後検知層の両方が参照する。
...
- タイミング: `PostToolUse`
- 対象: `Bash`
- 状態管理: `/tmp/cogmem-failure-count-{PPID}` に連続失敗カウントを保持（PPID = 親プロセスID。Claude Code プロセス単位で隔離）
- 成功（exit 0）でカウンタリセット
- プロセス終了時にファイルは自然に孤立する（次回起動時に PPID が変わるため衝突しない）

### 注入メッセージ

メッセージ内の回数は `consecutive_failure_threshold` の値を動的に埋め込む:

```
⚠ コマンドが{threshold}回連続で失敗しています。
...
どの cogmem プロジェクトでも共通のマッピング（`memory/logs/` は `cogmem init` が作成、`.claude/skills/` は Claude Code の標準ディレクトリ）。スキルが存在しない場合は照合がスキップされるため、未使用環境への影響はない。
...
タイムゾーン対応: SQLite の `localtime` 修飾子でシステムのローカル時刻に変換する。
...
wrap Step 0: cogmem watch --since "8 hours ago" --json
  → git diff HEAD~N..HEAD（--since 期間のコミット範囲）で変更ファイル一覧を取得
  → ステージ済み未コミット (git diff --cached --name-only) も含める
...
| failure-breaker 状態 | `/tmp/cogmem-failure-count-{PPID}` | プロセス単位で隔離 |
